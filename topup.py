...
from utils import generate_memo, safe_username, get_ton_rate_uah
...
def topup_ton_handler(message):
    ...
    bot.send_message(
        ...
        (
            ...
            f"Максимум: <b>{MAX_TOPUP_TON} TON</b>\n"
            f"Курс: <b>1 TON = {ton_rate:.2f} грн</b>"
        ),
        reply_markup=back_only_menu()
    )
...
def topup_callback_handler(call):
    ...
    if call.data == "topup:confirm":
        ...
        if method == "Карта (UA)":
            ...
            ton_rate = state.get("ton_rate", 210.0)
            pretty_ton = f"{amount:.4f}".rstrip("0").rstrip(".")

            text = (
                "<b>💎 Реквізити для поповнення</b>\n\n"
                f"Сума до оплати: <b>{pretty_ton} TON</b>\n"
    ...
    if call.data == "topup_paid":
        ...
        pretty_amount = (
            f"{amount:.4f}".rstrip("0").rstrip(".")
            if method == "Криптовалюта (TON)"
            else f"{amount:.2f}"
        )
