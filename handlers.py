...
def check_sub_callback(call):
    if is_user_subscribed(call.from_user.id):
        ...
        bot.send_message(call.message.chat.id, "Оберіть потрібний розділ нижче.", reply_markup=main_menu())
    else:
...
def back_handler(message):
    ...
    if action == "buy_friend_amount":
        ...
        bot.send_message(message.chat.id, "Введіть username отримувача у форматі @username:", reply_markup=back_only_menu())
        return
